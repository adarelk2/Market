...
{
   _ar.items.map(items=>{
      let product = new ProductClass("#id_items",items.id,items.name,items.subject,items.price,items.cate);
      product.adminRender();
    })
}
else
{
   ...
   _ar.items.map(items=>{
      let product = new ProductClass("#id_items",items.id,items.name,items.subject,items.price,items.cate,items.img);
      product.render();
    })
       }
}
...
_obj.map(items=>{
let product = new ProductClass("#id_cartItems",items.id,items.name,items.subject,items.price);
