...
class User
{
   ...
   function Register($_email,$_user,$_password)
   {
      $message = "Register was faild!";
      $sql = "Select userName,email from users where userName='$_user'";
      if($this->checkColmn($sql) ==false)
      {
...
function makeLoading($_count)
{
   ...
   function checkColmn($_query)
   {
      $count = $this->mysqli->query($_query)->num_rows;
      if($count >=1)
         return true;
   }
}
